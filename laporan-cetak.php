...
class PDF extends FPDF{
	...
	function PDF($orientation='L', $unit='mm', $size='Letter'){
	...
	function Header(){
	    ...
	    $this->Cell(80);
	    $this->Cell(30,10,'LAPORAN SISTEM PENDUKUNG KEPUTUSAN',0,0,'J');
	    $this->Ln(20);
	}
...
while ($row2x = $stmt2x->fetch(PDO::FETCH_ASSOC)){
	$pdf->Cell(45,7,$row2x['nama_kriteria'],1,0,'L');
}
...
while ($row1x = $stmt1x->fetch(PDO::FETCH_ASSOC)){
	...
	while ($rowrx = $stmtrx->fetch(PDO::FETCH_ASSOC)){
		$pdf->Cell(45,7,$rowrx['nilai_rangking'],1,0,'L');
	}
	$pdf->ln();
}
...
while ($row2y = $stmt2y->fetch(PDO::FETCH_ASSOC)){
	$pdf->Cell(45,7,$row2y['nama_kriteria'],1,0,'L');
}
...
while ($row1y = $stmt1y->fetch(PDO::FETCH_ASSOC)){
	...
	while ($rowry = $stmtry->fetch(PDO::FETCH_ASSOC)){
		$pdf->Cell(45,7,number_format($rowry['nilai_normalisasi'], 3, '.', ','),1,0,'L');
	}
	$pdf->ln();
}
$pdf->Cell(40,7,'Bobot',1,0,'L');
while ($row2yx = $stmt2yx->fetch(PDO::FETCH_ASSOC)){
	$pdf->Cell(45,7,$row2yx['bobot_kriteria'],1,0,'L');
}
$pdf->ln();
...
$pdf->SetFont('Times','B',12);
$pdf->Cell(20,7,'K/A',1,0,'L');
while ($row2 = $stmt2->fetch(PDO::FETCH_ASSOC)){
	$pdf->Cell(43,7,$row2['nama_kriteria'],1,0,'L');
}
$pdf->Cell(32,7,'Hasil',1,0,'L');
$pdf->ln();
$pdf->SetFont('Times','',12);

while ($row1 = $stmt1->fetch(PDO::FETCH_ASSOC)){
	$pdf->Cell(20,7,$row1['nama_alternatif'],1,0,'L');
	$stmtr = $pro->readR($row1['id_alternatif']);
	while ($rowr = $stmtr->fetch(PDO::FETCH_ASSOC)){
		$pdf->Cell(43,7,number_format($rowr['bobot_normalisasi'], 3, '.', ','),1,0,'L');
	}
	$pdf->Cell(32,7,$row1['hasil_alternatif'],1,0,'L');
	$pdf->ln();
}
